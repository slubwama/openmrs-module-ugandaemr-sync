...
public class ReceiveViralLoadResultFromCentralServerTask extends AbstractTask {
    ...
    public void execute() {
        ...
        for (SyncTask syncTask : ugandaEMRSyncService.getIncompleteActionSyncTask()) {
            ...
            String dataOutput = generateVLFHIRResultRequestBody(VL_RECEIVE_RESULT_FHIR_JSON_STRING, ugandaEMRSyncService.getHealthCenterCode(), ugandaEMRSyncService.getPatientARTNO(viralLoadRequestEncounter.getPatient()), String.valueOf(syncTask.getSyncTask())).get("json");
            ...
            if (results != null && results.size() > 0 && UgandaEMRSyncUtil.getSuccessCodeList().contains(results.get("responseCode"))) {
                ...
                String dateFormat = ugandaEMRSyncService.getDateFormat(viralLoadRequestEncounter.getEncounterDatetime().toString());
                ...
                EncounterSearchCriteria encounterSearchCriteria = new EncounterSearchCriteria(viralLoadRequestEncounter.getPatient(), null, ugandaEMRSyncService.convertStringToDate(viralLoadRequestEncounter.getEncounterDatetime().toString(), "00:00:00", dateFormat), ugandaEMRSyncService.convertStringToDate(viralLoadRequestEncounter.getEncounterDatetime().toString(), "23:59:59", dateFormat), null, null, encounterTypes, null, null, null, false);
                ...
                if (encounters.size() > 0) {
                    ugandaEMRSyncService.addVLToEncounter(result.get("valueString").toString(), result.get("valueInteger").toString(), viralLoadRequestEncounter.getEncounterDatetime().toString(), encounters.get(0));
